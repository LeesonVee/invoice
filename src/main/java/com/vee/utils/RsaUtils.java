...
public class RsaUtils {
    ...
    public static void main(String[] args) throws Exception{
        //test_("600120","92fc24df9773d59a04b34091d1a868c277a45d44708d22fca4fbdbb739105a85");
    ...
    public static String getIdCardNo(String appId,String loginCode) throws Exception{
        SimpleDateFormat df = new SimpleDateFormat("yyyyMMddHHmmss");
        String timestamp=df.format(new Date())+"000";
        StringBuffer signStr = new StringBuffer(0);
        signStr.append("corpid=").append(CORP_ID).append("&timestamp=").append(timestamp);
        String sign=Signature.rsaSign(signStr.toString(), PRI_KEY, "UTF-8");
        ...
        body.append("\"CorpId\":\"").append(CORP_ID).append("\",");
    ...
    public static String pubEncode(String plainText)throws Exception{
        //公钥加密过程
        byte[] cipherData= RSAEncrypt.encrypt(RSAEncrypt.loadPublicKeyByStr(PUB_KEY),plainText.getBytes());
        return com.vee.rsa.Base64.encode(cipherData);
    }
    public static String priEncode(String plainText)throws Exception{
        //私钥加密过程
        byte[] cipherData=RSAEncrypt.encrypt(RSAEncrypt.loadPrivateKeyByStr(PRI_KEY),plainText.getBytes());
        return com.vee.rsa.Base64.encode(cipherData);
    }
    public static String pubDecode(String cipher)throws Exception{
        //公钥解密过程
        byte[] res=RSAEncrypt.decrypt(RSAEncrypt.loadPublicKeyByStr(PUB_KEY), com.vee.rsa.Base64.decode(cipher));
        return new String(res);
    }
    public static String priDecode(String cipher)throws Exception{
        //私钥解密过程
        byte[] res=RSAEncrypt.decrypt(RSAEncrypt.loadPrivateKeyByStr(PRI_KEY), com.vee.rsa.Base64.decode(cipher));
        return new String(res);
    }
}
